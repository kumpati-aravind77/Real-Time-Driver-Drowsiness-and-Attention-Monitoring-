#!/usr/bin/env python3
"""
AIS-184 Complete DDAWS (single-file) — audio-enabled:
 - All L1 and L2 warnings now play sound
 - Multi-backend audio: simpleaudio -> pygame -> aplay (Linux)
...
class MultiAudioPlayer:
    ...
    @classmethod
    def init(cls, wav_path: str):
        cls.wav_path = wav_path
        if not os.path.isfile(wav_path):
            print("[Audio] beep file not found:", wav_path)
            cls.backend = None
            return
        if SIMPLEAUDIO_AVAILABLE:
            try:
                cls.wave_obj = sa.WaveObject.from_wave_file(wav_path)
                cls.backend = "simpleaudio"
                print("[Audio] Using simpleaudio backend.")
                return
            except Exception as e:
                print("[Audio] simpleaudio init failed:", e)
        if PYGAME_AVAILABLE:
            try:
                pygame.mixer.init()
                ...
                print("[Audio] pygame init failed:", e)
        if sys.platform.startswith("linux") and shutil_which("aplay"):
            cls.backend = "aplay"
            print("[Audio] Using aplay backend.")
            return
        cls.backend = None
        print("[Audio] No audio backend available. Sound disabled.")

    @classmethod
    def play(cls):
        if cls.backend is None: return
        if cls.backend == "simpleaudio" and cls.wave_obj is not None:
            try: cls.wave_obj.play()
            except: pass
        elif cls.backend == "pygame":
            threading.Thread(target=_pygame_play, daemon=True).start()
        elif cls.backend == "aplay":
            threading.Thread(target=_aplay_play, daemon=True).start()
...
def _aplay_play():
    try: subprocess.run(["aplay", "-q", MultiAudioPlayer.wav_path], check=False)
    except: pass
...
# -------------------- CLI & Main --------------------
def parse_args():
    p = argparse.ArgumentParser(description="AIS-184 DDAWS — audio-enabled")
    p.add_argument("--camera", type=int, default=0)
    p.add_argument("--beep", type=str, default="./beep.wav")
    p.add_argument("--no-audio", action="store_true")
    p.add_argument("--calibrate", action="store_true")
    p.add_argument("--ear", type=float, default=CONFIG.ear_threshold)
    p.add_argument("--mar", type=float, default=CONFIG.mar_threshold)
    p.add_argument("--perclos-l1", type=float, default=CONFIG.perclos_warn_level1)
    p.add_argument("--perclos-l2", type=float, default=CONFIG.perclos_warn_level2)
    return p.parse_args()

def main():
    args = parse_args()
    if not args.no_audio:
        MultiAudioPlayer.init(args.beep)
